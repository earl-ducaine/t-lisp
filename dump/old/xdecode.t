...
;;; (read-string! dump-stream n string)
;;; ==================================================================
;;; Read in n bytes, lower byte first, storing characters into a string.
;;; Assuming there is no EOF when calling.
;;;

(define-operation (read-string! dump-stream n string))

;;; (read-buffer dump-stream n)
;;; ==================================================================
;;; Read in n bytes, lower byte first,  and return a buffer.
;;; Assuming there is no EOF when calling.
;;;

(define-operation (read-buffer dump-stream n))
...
;;; Assuming that calling has no error for efficiency.
;;;
...
(define (open-dumped file)
    (let* ((string-vec '())
      ...
      (object nil
        ...
        ((read-string! self n string)
         (loop (incr i from 0 to (fx- n 1))
               (do (set (nthchar string i) (read1c stream))))
         string)

        ((read-buffer self n)
         (read-string! self n (get-buffer-of-size n)))
        ...
        ((do-symbol self)
         (let ((buf (read-buffer self
                                 (get-fixnum self type %-symbol))))
           (block0 (string->symbol buf)
                   (release-buffer buf))))

        ((do-symbol-shared self)
         (let ((buf (read-buffer self
                                 (get-fixnum self type %-symbol-shared))))
        ...
        ((do-string self)
         (let ((n (get-fixnum self type %-string)))
           (read-string! self n (make-string n))))

        ((do-string-shared self)
         (let ((n (get-fixnum self type %-string-shared)))
