...
(define (transform node)
  (cond ((not (call-node? node))
        ...
        ((lambda-node? (call-proc node))
         (transform-let node))
        ((known-primop-value (call-proc node))
         => (lambda (primop)
              (if (or (null? (primop.formals primop))
                      (primop.constructed? primop))
                  (primop.transform primop node)
                  nil)))
        (else
         nil)))

;;; (TRANSFORM-LET let-node)
;;;========================================================================
;;;  ((lambda () x)) => x
...
(define (transform-let let-node)
  (let ((node (call-proc let-node)))
    (cond ((and (null? (lambda-variables node))
          ...
          ;;;   (cdr (variable-refs (lambda-cont-var node)))
           (transform-join let-node))
          (else
           (*or (any? true?
                      (map (lambda (var val)
                             (cond ((not (used? var))
...
(define (transform-join join-node)
    ...
    (destructure (((cond? exit1 exit2 test? #f ref)
      ...
      (cond ((and (fx= 1 (length (lambda-variables cont)))
             ...
             (let* ((c1 (create-variable 'c))
               ...
               (walk (lambda (ref)
                       (replace ref (copy-node-tree cont '() nil)))
...
(define (transform-receive-values node)
...
(define (transform-to-conditional node)
