...
(define (do-trivials call-node node reg closure-reg)
  (cond ((lambda-node? node)
         (let ((offset (environment-cic-offset (lambda-env node))))
           (cond ((fx= offset 0)
                  (generate-move call-node closure-reg reg))
                 (else
                  (generate-move-address call-node
                           (reg-offset closure-reg offset)
                           reg)))))
        (else
         (generate-move call-node (access-value call-node (leaf-value node))
                                  reg))))
...
(define (do-indirect node pair)
  (cond ((lambda-node? (car pair))
         ...
         (generate-move-address node
                               ...
                               (cdr pair)))
        (else
         (generate-move node (access-value node (car pair))
                             (cdr pair))
            ...
            (else
             (iterate loop2 ((candidates targets))
               (cond ((null? candidates)
                      (let ((pair (car pairs)))
                        (generate-move node (cdr pair) TN)
                        (generate-move node (car pair) (cdr pair))
                        (loop1 (cdr pairs)
...
(define (make-stack-closure node cont)
  ...
  (walk (lambda (pair)
          (generate-push node (access-value node (car pair))))
        (reverse (cdr (closure-env (environment-closure (lambda-env cont))))))
  (generate-push-address node (template cont)))
...
(define (make-heap-closure node closure)
  ...
  (let* ((members (closure-members closure))
    ...
    (generate-move-address node (template (variable-binder first))
                                AN)
    ...
    (walk (lambda (pair)
      (let ((var (car pair))
        ...
        (cond ((memq? var members)
               (generate-move-address node (template (variable-binder var))
                                      (reg-offset AN
                                                  (fx- offset tag/extend))))
              (else
               (generate-move node (access-value node var)
                                   (reg-offset AN
                                               (fx- offset tag/extend)))))))
...
(define (generate-extend node n)                      
  ...
  (generate-move node (lit n) S1)
  (lock AN)
  (generate-move node
                 (access-value node primop/%make-extend)
                 AN-1)
  (unlock AN)
...
;;; which can be either '* or a specific register.
  ...
  (cond ((reg-node reg)
         => (lambda (value)
              (let ((new (where-used value)))
                (cond ((and new (get-reg-if-free new))
                       => (lambda (new)
                            (mark value new)
                            (set (reg-node reg) nil)
                            (generate-move node reg new)))
                      (else
