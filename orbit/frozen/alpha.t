...
(define-exported-compilator (lambda vars . body) (syntax shape)
  (alpha-lambda 'p (cons 'k vars) body syntax shape))
  ...
  (do ((l var-names (cdr l))
      ...
      ((atom? l)
       (let* ((vars (map (lambda (name)
                         ...
                         (cons l (reverse! z))))
              (node (create-lambda-node proc-name vars)))
         (relate lambda-body node
                 (alpha-block body syntax (bind-variables vars node shape)))
         node))))
...
(define-exported-compilator (define-constant var value) (syntax shape)
  (cond ((atom? var)
        ...
        (else
         (alpha (syntax-error "illegal form~%  (DEFINE-CONSTANT ~S ~S)"
                              var
                              value)
                syntax shape))))
...
;;; (block a . b)
;;;   ==>  (receive-values (lambda ignore . b) (lambda () a))
  ...
  (cond ((null? (cdr exp-list))
        ...
        (else
         (let ((node (alpha (car exp-list) syntax shape)))
           (alpha-call `(,primop/receive-values
                         (,syntax/lambda ignore . ,(cdr exp-list))
                         (,syntax/lambda () ,node))
                       syntax
                       shape)))))

;;; (receive vars exp . body)
;;;   ==>  (receive-values (lambda vars . body) (lambda () exp))

(define-exported-compilator (receive vars exp . body) (syntax shape)
  (alpha-call `(,primop/receive-values
...
;;; Makes an entry in syntax-table SYNTAX for SPEC.
...
;;; =>
;;; (LAMBDA ARGS
;;;   (PRIMOP/PROC+HANDLER <proc>
;;;                        (LAMBDA () (VALUES <op1> ... <opN>))
;;;                        (LAMBDA (() () () . <args1>) . <method1>)
;;;                           ...
;;;                        (LAMBDA (() () () . <argsN>) . <methodN>)
;;;                        <default>))

(define-exported-compilator (object proc . specs) (syntax shape)
  (let ((proc-node (alpha proc syntax shape)))
    (receive (ok operations-node methods-nodes)
             (parse-object-specs specs syntax shape)
      (cond ((not ok)
             (alpha (syntax-error "illegal form (OBJECT ~S" `(,proc . ,specs))
                    syntax shape))
            (else
             (alpha `(,syntax/lambda args
                        (,primop/proc+handler
                         ,proc-node
                         ,operations-node
                         . ,methods-nodes))
...
(define (parse-object-specs specs syntax shape)
  (iterate loop ((specs specs) (ops '()) (handlers '()))
    (cond ((null? specs)
           (values t
                   (alpha `(,syntax/lambda ()
                              (,primop/values . ,(reverse! ops)))
                          syntax
                          shape)
                   (reverse! handlers)))
          ((pair? (caar specs))
           (destructure ((((op . vars) . body) (car specs)))
             (loop (cdr specs)
                   (cons op ops)
                   (cons `(,syntax/lambda (() () () . ,vars) . ,body)
                          handlers))))
          ((or (neq? (caar specs) '=>)
               (cdr specs))
           (values nil nil nil))
          (else
           (loop '() ops (cons (cadar specs) handlers))))))
