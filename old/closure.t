...
(define (live-analyze-leaf node global)
  (cond ((and (literal-node? node)
              (not (immediate? (literal-value node)))
              (not (memq? (literal-value node) *unit-literals*)))
         (push *unit-literals* (literal-value node))
         `(,global))
        ((and (primop-node? node)
              (primop.closed-compiled? (primop-value node)))
         `(,global))
  ...
  (let ((proc (call-proc node)))
    (cond ((and (primop-node? proc)
          ...
          ((and (primop-node? proc)
                (eq? (primop-value proc) primop/*block))
           (live-analyze-block ((call-arg 1) node) ((call-arg 2) node) global))
          ((lambda-node? proc)
...
(define (live-analyze-let node global)
  (walk (lambda (var node)
        ...
        (cdr (lambda-variables (call-proc node)))
        (call-args node))
...
(define (live-analyze-block cont body global)
  (set (lambda-strategy body) 'strategy/open)
  (cond ((primop-node? (call-proc (lambda-body body)))
         (set (variable-type (cadr (lambda-variables body))) cont)
         (set (lambda-strategy cont) 'strategy/open))
        (else
         (set (lambda-strategy cont) 'strategy/stack)))
  (union (live-analyze cont global) (live-analyze body global)))
...
(define (live-analyze-Y cont-node lambda-node global)
  (let* ((label-exprs (cdr (call-args (lambda-body lambda-node))))
    ...
    (walk (lambda (label-var expr)
          ...
          (cddr (lambda-variables lambda-node))
          label-exprs)
...
(define (close-analyze-top node free-variables defined-variables)
    ...
    (let* ((l ((call-arg 1) (lambda-body node)))
           (env (list (car (lambda-variables node))))
...
(define (really-close-analyze-body nodes senv svia henv hvia)
  (receive (live cics)
    ...
    (cond (cics
           (let ((link (if (intersection? live henv) hvia nil))
             ...
             (walk (lambda (cic)
                     (close-analyze-body (lambda-body cic)
                                         ...
                                         (lambda-self-var cic)))
                   cics)
             (create-closure link (map lambda-self-var cics) delta)))
          (else nil))))
...
(define (close-analyze-stack node stackenv stackvia heapenv heapvia)
  (let* ((live (lambda-live node))
    ...
    (create-closure (if (and link-set (eq? stackvia heapvia)) heapvia nil)
                    (list (car (lambda-variables node)))
                    closure-env)
    ...
    (receive (pointer scratch)
      ...
      (let ((scratch-slots (length scratch))
          ...
          (set (closure-size       closure)
               (fx* (fx+ scratch-slots pointer-slots) 4))
