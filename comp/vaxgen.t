  ...
  (cond ((lambda-node? value)
        ...
        (else
         (let ((access (access-value node value)))
           ...
           (generate-move access (access-locative node location))
          ...
          (else
           (let ((access (access-value node (leaf-value value))))
             ...
             (let ((loc-reg (->register 'pointer node loc '*)))
               (generate-move access (reg-offset loc-reg offset))
               (release-access access)))))))
  ...
  (destructure (((cont arg) (call-args node)))
    (let* ((rwl (result-want-loc cont))
      ...
      (cond ((dying? var node)
             ...
             (kill var)
             (generate-move (reg-offset dest offset) dest))
            (else
             (lock dest)
             (let ((reg (->register 'pointer node var '*)))
               (unlock dest)
               (free-register node dest)
               (generate-move (reg-offset reg offset) dest))))
...
(define (generate-call-with-continuation node)
  (destructure (((cont proc) (call-args node)))
    ...
    (generate-move SP (reg-offset A5 escape-closure/cont))
    (generate-move (access-name 'global/dynamic-state)
                        (reg-offset A5 escape-closure/state))
    (mark (cadr (lambda-variables proc)) A5)
    (allocate-call (lambda-body proc))))


(define (generate-foreign-call node xenoid param-list value-rep)
  (let* ((args (reverse (cdr (call-args node))))
         (nargs (length args))
         (cont (car (call-args node))))
    ;(generate-gc-defer)
    (walk (lambda (arg param)
            (rep-push (leaf-value arg) (cadr param)))
          args
          param-list)
    (free-register node S0)                           ; this must be R0
    (let ((reg (->register 'pointer node xenoid '*)))
      (emit vax/movl (reg-offset reg tag/extend) reg)
      (emit vax/calls (machine-num nargs) (reg-offset reg 0))
      (kill xenoid)
      (n-decrement-stack nargs)
      (cond ((lambda-node? cont)
             (walk (lambda (node)
                      (kill-if-dead node cont))
                   args)
             (let* ((rwl (result-want-loc cont))
                    (reg (cond ((register? rwl) rwl)
                               ((eq? rwl 'scratch) S0)
                               (else reg))))
               (free-register node reg)
               (if value-rep
                   (really-rep-convert S0 value-rep reg
                                       (rep (car (lambda-variables cont)))))
               (mark (car (lambda-variables cont)) reg)))
            (else
             (if value-rep
                 (really-rep-convert S0 value-rep A1 'rep/pointer)))))))
